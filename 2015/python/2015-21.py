...
@pytest.mark.parametrize(
    "items,combo_range,expected",
    [
        (["a", "b"], range(1), []),
        (["a", "b"], range(2), [(None,), ("a",), ("b",)]),
        (["a", "b"], range(1, 2), [("a",), ("b",)]),
        (["a", "b"], range(3), [(None, None), ("a", None), ("b", None), ("a", "b")]),
        (["a", "b"], range(1, 2), [("a",), ("b",)]),
        (["a", "b"], range(2, 3), [("a", "b")]),
    ],
)
...
@pytest.mark.parametrize(
    "fighter,enemy,expected_damage",
    [
        (Fighter(hit_points=8, damage=5, armor=5), Fighter(0, 0, 2), 3),
        (Fighter(hit_points=8, damage=7, armor=2), Fighter(0, 0, 5), 2),
        (Fighter(hit_points=0, damage=0, armor=0), Fighter(0, 0, 10), 1),
    ],
)
