...
def total_presents(house_number, presents_per_elf, elf_limit=None):
    ...
    divisors = {divisor for divisor_pair in divisors for divisor in divisor_pair}
    if elf_limit is not None:
        divisors = (d for d in divisors if d * elf_limit >= house_number)
    return sum(d * presents_per_elf for d in divisors)


def first_house_with_n_presents(
    target_presents, head_start=50, elf_limit=None, presents_per_elf=10
):
    ...
    while presents < target_presents:
        ...
        presents = total_presents(
            house_number, elf_limit=elf_limit, presents_per_elf=presents_per_elf
        )
    return house_number


@pytest.mark.parametrize(
    "house_number,expected_presents",
    [
        (1, 10),
        (2, 30),
        (3, 40),
