...
input_file = (
    pathlib.Path(__file__).resolve().parent.parent.joinpath("input", "2015-15.txt")
)
...
Cinnamon: capacity 2, durability 3, flavor -2, texture -1, calories 3"""
    expected = dict(
        Butterscotch=dict(capacity=-1, durability=-2, flavor=6, texture=3, calories=8),
        Cinnamon=dict(capacity=2, durability=3, flavor=-2, texture=-1, calories=3),
    )
...
def brute_force_cookie(ingredients, teaspoons):
    ...
    all_scores = [
        cookie_score(amounts, ingredients) for amounts in combo_four(teaspoons)
    ]
...
def search_for_best_cookie(
    starting_recipe, ingredients, teaspoons, recipes_tried, cal_target=500
):
    ...
    def new_recipes(current_recipe):
        for c in single_change_permutations:
            ...
            suitable = (
                candidate not in recipes_tried
                and sum(candidate) == teaspoons
                and min(candidate) >= 0
                and max(candidate) <= teaspoons
            )
            if suitable:
                yield candidate
            else:
    ...
    def initial_check(starting_recipe):
        ...
        max_score = max(score_pairs, key=lambda t: t[1], default=(tuple(), 0, 0))
        return max_score
...
def repeated_search(ingredients, teaspoons, iterations):
    ...
    while iterations:
        ...
        if tuple(recipe) in visited:
            ...
            iterations -= 1
        score = search_for_best_cookie(recipe, ingredients, 100, recipes_tried=visited)
        if sum(score):
            scores.append(score)
    if not scores:
        return (0, 0)
    else:
        return (max(t[0] for t in scores), max(t[1] for t in scores))
